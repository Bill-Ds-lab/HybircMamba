...
OUTPUT_TXT_PATH = "missingAdditiveGaussianNoise_255.txt"
PREFIX_FILTER = "AdditiveGaussianNoise_255"
...
def load_model(checkpoint_path, device):
    ...
    clean_state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()} if isinstance(state_dict,
                                                                                                  dict) else state_dict
...
def main():
    ...
    print(f"\n🔍 Đang quét dataset để tìm các ảnh có tên bắt đầu bằng '{PREFIX_FILTER}' bị dự đoán SAI...")
    ...
    with open(OUTPUT_TXT_PATH, "w", encoding="utf-8") as f:
        f.write("=" * 85 + "\n")
        f.write(f"DANH SÁCH ẢNH NHIỄU ({PREFIX_FILTER}) BỊ DỰ ĐOÁN SAI\n")
        f.write(f"Tổng số ảnh nhiễu tìm thấy: {total_target_images:,} ảnh\n")
        f.write(f"Tổng số trường hợp bị đoán sai: {len(misclassified_list):,} ảnh\n")
        if total_target_images > 0:
            error_rate = (len(misclassified_list) / total_target_images) * 100
            f.write(f"Tỷ lệ lỗi trên tập ảnh nhiễu này: {error_rate:.2f}%\n")
        f.write("=" * 85 + "\n\n")
    ...
    print("\n" + "=" * 50)
    print(f"✅ Hoàn thành!")
    print(f"📊 Đã quét {total_target_images:,} ảnh thuộc nhóm nhiễu '{PREFIX_FILTER}'.")
