...
def get_or_create_split(full_dataset, save_path: str, dataset_name: str,
    ...
    if os.path.exists(split_path) and not force_recreate:
        print(f" Dùng lại split cố định đã lưu: {split_path}")
        ...
        def _resolve(path_list, part_name):
            ...
            for p in path_list:
                if p in path_to_sample:
                    ...
                    missing += 1
            if missing > 0:
                print(f": {missing} ảnh trong '{part_name}' đã lưu "
                      f"không còn tồn tại ")
            return resolved
    ...
    print(f"[SPLIT]  Chưa có file split cố định, đang tạo mới tại: {split_path}")
